...
function initializeDB() {
  try {
    db = window.openDatabase("cfs_apps.db", "1.0", "CFS Apps", 200000);
    //db = window.SQLitePlugin.openDB({name: "mydata.db", location: 'default'});
    //logTextToConsole("DB Opened");
    defineSchema();
  }
  catch(ex){
...
function defineSchema() {
  try{
    db.transaction(function(transaction){
      transaction.executeSql('CREATE TABLE IF NOT EXISTS settings (name text primary key, value text)', [],
      function(tx, result){
      ...
      function(error){
        //error occurred
        alert("Error executing create table");
      });
    });
  }
  catch(ex){
    alert("Error defining database schema");
  }
}
...
function getServer() {
  try{
    db.transaction(function(transaction){
      transaction.executeSql("SELECT value FROM settings WHERE name = 'server'", [],
      function(tx, result){
        //execution successful
        if(result.rows.length > 0){
          wsUri.value = result.rows.item(0).value;
        }
        else {
          wsUri.value = "ws://10.10.200.6:8001";
        }
      },
      function(error){
        //error occurred
        alert("Error executing retreive setting query");
      });
    });
  }
  catch(ex){
    alert("Error retreiving setting");
  }
}
...
function saveSetting(setting, value) {
  ...
  try{
    ...
    db.transaction(function(transaction){
      transaction.executeSql("REPLACE INTO settings (name, value) VALUES ('" + setting + "', '" + value + "')", [],
      function(tx, result){
        //execution successful
      ...
      function(error){
        //error occurred
        alert("Error executing save setting query");
      });
    });
  }
  catch(ex){
...
function initializeLocation() {
 ...
 // wsUri.value = getParameterByName('location');
  // get server value from database
  wsUri.value = getSetting('server', 'ws://10.10.200.6:3001');
  /*
  console.log("server: " + wsUri.value);
  if (wsUri.value != '') {
    return;
  }
  var wsScheme = 'ws:';
...
function onOpen(evt)
{
  ...
  saveSetting('server', wsUri.value);
...
//window.addEventListener('load', initializeDB, false);
